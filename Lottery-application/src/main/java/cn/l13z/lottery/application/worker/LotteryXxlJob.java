    ...
    public void lotteryOrderMQStateJobHandler() throws Exception {
        ...
        for (String param : params) {
            ...
            for (int i = 0; i < tbCount; i++) {
                ...
                // 扫描库表数据
                List<InvoiceVO> invoiceVOList = activityDeploy.scanInvoiceMqState(dbCount, i);
                logger.info("扫描用户抽奖奖品发放MQ状态[Table = 2*4] 扫描库：{} 扫描表：{} 扫描数：{}", dbCount, i,
                    invoiceVOList.size());

                // 补偿 MQ 消息
                for (InvoiceVO invoiceVO : invoiceVOList) {

                    ListenableFuture<SendResult<String, Object>> future = sendLotteryInvoice.sendLotteryInvoice(invoiceVO);
                    future.addCallback(new ListenableFutureCallback<SendResult<String, Object>>() {

                        @Override
                        public void onSuccess(SendResult<String, Object> stringObjectSendResult) {
                            // MQ 消息发送完成，更新数据库表 user_strategy_export.mq_state = 1
                            activityDeploy.updateInvoiceMqState(invoiceVO.getuId(),
                                Long.valueOf(invoiceVO.getOrderId()),
